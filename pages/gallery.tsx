          ...
          >
            {nfts &&
              nfts?.map((nft: NFTType, idx: number) => {
                ...
                if (isSafe.success) {
                  return (
                    <WrapItem key={nft.id} border={'4px solid black'}>
                      <NFTCard
...
function NFTCard({
  ...
  setSelectedNFTId: React.Dispatch<React.SetStateAction<number>>;
  idx: number;
  onOpen: () => void;
  nft: NFTType;
  imgLoaded: boolean;
  setImgLoaded: React.Dispatch<React.SetStateAction<boolean>>;
  handleFavorite: (nft: NFTType) => Promise<Response>;
  nfts: NFTType[];
  mutate: () => void;
  toast: any;
}) {
      ...
      >
        ...
        <Stack spacing='3' py={4} pt={0}>
          <Skeleton
            ...
            isLoaded={imgLoaded}
            w={imgLoaded ? 'unset' : Math.max(120, Math.random() * 300) + 'px'}
          ></Skeleton>
          ...
          <SkeletonCircle isLoaded={imgLoaded}>
            <IconButton
              ...
              onClick={async () => {
                const param =
                  nfts &&
                  (nfts.find((n: NFTType) => n.id === nft.id) as NFTType);

                const res = await handleFavorite(param);
              ...
              icon={nft.isFavorite ? <Text>♥</Text> : <Text>♡</Text>}
            />
          </SkeletonCircle>
          <Text
            color='black'
